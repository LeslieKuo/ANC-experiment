...
def plotfft(wave_data,framerate,title,maxY = 10000):#maxY=20000
    ...
    pylab.title(title)
    pylab.show()
def plotfft2w(wave_data,framerate,title,maxY = 2000):#maxY=20000
    ...
    ax = subplot(111)
    xmajorLocator = MultipleLocator(2000)  # 将x主刻度标签设置为20的倍数
    xmajorFormatter = FormatStrFormatter('%1.1f')  # 设置x轴标签文本的格式
    xminorLocator = MultipleLocator(1000)  # 将x轴次刻度标签设置为5的倍数
    # 设置主刻度标签的位置,标签文本的格式
    ax.xaxis.set_major_locator(xmajorLocator)
    ax.xaxis.set_major_formatter(xmajorFormatter)
    # 显示次刻度标签的位置,没有标签文本
...
def phase_shift(signal,phase):
    ...
    # xnew = np.arange(0, len(signal) - 1, phase) #这是八倍点都取出来
    ynew = f(xnew)
    return ynew
#
...
def align_timeline(su,sv,testnum):
    ...
    sv_ = sv[testnum:]
    value_ave = np.average(np.abs(su_ - sv_))
    min_result = (testnum, value_ave)
    return min_result,su_,sv_

#8倍插值的结果，返回类型为int16，可以直接写入音频文件（phase_shift唯一区别就是格式）
def test_interpolate_wav(signal,phase):
    assert 0 <= phase <= 1
    x = np.arange(0, len(signal))
    y = np.array(signal)
    f = interpolate.interp1d(x, y)
